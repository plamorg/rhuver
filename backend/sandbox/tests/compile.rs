...
fn compile_test_success() {
    let path: String = env!("CARGO_MANIFEST_DIR").to_string() + "/tests/test.cpp";
    sandbox::compile("/usr/bin/g++".to_string(), vec!["g++", "-std=c++17", "-O2", "-w", &path, "-o", "test"].iter().map(|x| x.to_string()).collect::<Vec<String>>()).unwrap();
